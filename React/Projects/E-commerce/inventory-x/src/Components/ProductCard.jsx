...
const ProductCard = ({ product }) => {
  const { addToCart, currentUser } = useContext(AppContext)
  ...
  return (
    <div className='bg-white border border-gray-100 hover:shadow-2xl hover:-translate-y-1 transition duration-300 w-64 rounded-2xl p-4 flex flex-col shadow-sm relative'>
      {product.stock <= 5 && product.stock > 0 && (
        <span className="absolute top-2 left-2 bg-orange-100 text-orange-600 text-xs font-bold px-2 py-1 rounded-full z-10">
